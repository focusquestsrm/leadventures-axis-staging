import type { Buyer, BuyerCap, BuyerProgram, Campaign, DeliveryAttempt, Lead, LeadRejection, Program, TrafficSource } from '../types'
...
export interface SourcePerformance { sourceId: string; source: string; campaignId: string; campaign: string; leads: number; accepted: number; rejected: number; acceptanceRate: number | null; averageResponseMs: number | null }
export interface BuyerScorecard { buyerId: string; buyer: string; attempted: number; accepted: number; rejected: number; acceptanceRate: number | null; averageResponseMs: number | null; cap: number; delivered: number; remaining: number; utilization: number | null; capacityStatus: CapacityStatus; topRejectionReason: string; revenue: number | null }
export interface ProgramPerformance { programId: string; program: string; leads: number; attempts: number; accepted: number; rejected: number; acceptanceRate: number | null; activeBuyers: number; remainingCapacity: number; topRejectionReason: string; revenue: number | null }
...
export function calculateIntelligence(input: IntelligenceInput, filters: IntelligenceFilters, now = new Date()): IntelligenceReport {
  ...
  return { tenantId: input.tenantId, range, kpis, funnel, trends, sources: sources.sort((a, b) => b.leads - a.leads), buyers, programs, rejectionCategories: [...categories.values()].sort((a, b) => b.count - a.count), rejectionBuyers: namedCounts(currentRejections, 'buyer'), rejectionPrograms: namedCounts(currentRejections, 'program'), recoverableSources, recoverablePrograms: namedCounts(currentRejections, 'program', true), dataQuality: { missingSource: currentLeads.filter((row) => !row.trafficSourceId).length, missingCampaign: currentLeads.filter((row) => !row.campaignId).length, missingProgram: currentLeads.filter((row) => !row.programId).length, missingResponseTime: currentAttempts.filter((row) => row.responseTimeMs == null).length, unknownRejectionReason: currentRejections.filter((row) => !row.category.trim()).length } }
}
