...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const selectTenant = useCallback((nextTenantId: string) => {
    ...
    setData((current) => current ? { ...current, programs: [], trafficSources: [], campaigns: [], leads: [], leadIdentities: [], buyers: [], offers: [], buyerPrograms: [], buyerRules: [], buyerCaps: [], leadDeliveries: [], deliveryAttempts: [], leadRejections: [], leadStatusHistory: [], leadOutcomes: [], integrations: [], tenantSettings: [], auditEvents: [] } : current)
...
function SignIn() {
  ...
  return <main className="auth-page"><section className="auth-brand"><div className="brand-mark large" aria-hidden="true">LV</div><span>Lead Ventures</span><strong>AXIS</strong><p>Intelligence for Lead-Driven Growth</p><small>Secure intelligence for modern lead operations.</small></section><section className="auth-card"><span className="eyebrow">Secure staging access</span><h1>Welcome back</h1><p>Sign in to your authorized Axis workspace.</p><form onSubmit={(event) => void submit(event)}><label>Email<input name="email" type="email" autoComplete="email" required /></label><label>Password<input name="password" type="password" autoComplete="current-password" required /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-button" disabled={busy}>{busy ? 'Signing in…' : 'Sign in'}</button></form></section></main>
}
