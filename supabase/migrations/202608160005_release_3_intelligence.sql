...
begin
  ...
  select jsonb_build_object(
    ...
    'trends',coalesce((select jsonb_agg(jsonb_build_object('label',day,'leads',leads,'accepted',accepted,'rejected',rejected,'acceptanceRate',case when leads>0 then round(accepted::numeric/leads*100,1) end) order by day) from (select received_at::date day,count(*) leads,count(*) filter(where current_status in ('accepted','recovered')) accepted,count(*) filter(where current_status='rejected') rejected from current_leads group by received_at::date) t),'[]'::jsonb),
